...
import { Heart, Send, CheckCircle2, ChevronLeft, RotateCcw } from 'lucide-react';
...
export const DuaCard: React.FC<DuaCardProps> = ({ guestName, onPrev, onGoToFirst }) => {
    ...
    <div className="relative w-full h-full flex flex-col justify-between p-6 sm:p-8 text-center select-none overflow-hidden">
      ...
      <div className="relative z-10 pt-2">
        <div className="flex items-center justify-center gap-1.5 text-[#2c4227]">
          ...
          <p className="text-[11px] sm:text-xs font-serif tracking-[0.28em] uppercase font-bold">
            Prayers & Blessings
          </p>
        </div>

        <p className="font-serif italic text-xs sm:text-sm text-[#283d24] mt-2 max-w-xs mx-auto leading-relaxed">
          “May Allah bless this sacred union with eternal peace, happiness, and abundant barakah.”
        </p>

        <div className="h-[1px] w-20 mx-auto bg-gradient-to-r from-transparent via-[#bfa378] to-transparent my-3" />
      </div>

      {/* CENTER: Interactive Guestbook */}
      <div className="relative z-10 my-1 max-w-[320px] mx-auto w-full">
        {sentMessage ? (
          <div className="p-4 rounded-2xl bg-[#eef5eb] border border-[#86a87e]/60 text-center shadow-xs animate-in fade-in zoom-in duration-300">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center justify-center gap-1.5">
              {quickChips.map((chip, idx) => (
                <button
                  ...
                  className="px-2.5 py-1 rounded-full bg-[#f0e8d8] hover:bg-[#e6dccb] border border-[#c5a880]/70 text-[10px] font-serif text-[#142312] font-semibold cursor-pointer transition-all active:scale-95 shadow-2xs"
                >
                  {chip}
                </button>
              ))}
            ...
            >
              <input
                ...
                className="w-full px-3.5 py-2 text-xs rounded-xl bg-[#f8f5ee] border border-[#c5a880]/60 text-[#12200f] placeholder:text-[#8a9985] focus:outline-hidden focus:border-[#855e1a] shadow-inner font-sans"
              />
              <button
                type="submit"
      ...
      <div className="relative z-10 pt-2 border-t border-[#c5a880]/40 flex items-center justify-between w-full max-w-[320px] mx-auto px-2 text-[10.5px] font-serif uppercase tracking-wider">
        ...
        <button
          type="button"
          onClick={onGoToFirst}
          className="flex items-center gap-1 text-[#855e1a] hover:text-[#5a3e0f] font-semibold cursor-pointer py-1"
        >
          <RotateCcw className="w-3 h-3 text-[#855e1a]" />
